...
async function tests() {
...

  {
    ...
    //! @eranhirsch - fuzzyMatchWords > legacy > Partial queries yield lower scores
    test(
...

  {
    ...
    var tmp = fuzzysort.go("obj", tmpObjs, {
      keys: ["s.s", "arr.0.o"],
      scoreFn(a) {
        return (a[0] ? a[0].score : 1) + (a[1] ? a[1].score : 1);
      },
    })[0];
    ...
    var results = fuzzysort.go("farzher", [{ yes: "farzher", no: "no" }], {
      keys: ["yes", "no"],
    });
